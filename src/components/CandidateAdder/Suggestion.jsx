        ...
        <div className="p-2 rounded" style={{position:"absolute",float:"bottom",backgroundColor:"white",zIndex:"1",maxHeight:"150px",overflowY:"scroll",width:"100%"}}>
            {
                props.suggestions.map(each=>
                    <div className="border" onClick={(e)=>{props.handleSuggestions(props.unique,props.name,each.skill||each.name||each.suggestion,e)}} >
                        {each.logo&&<img className="p-2 m-0" src={each.logo} style={{width:"50px",height:"50px",}} />}
